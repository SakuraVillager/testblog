...
:::tip
由于是一边试验一边写博客的，所以最开始的git子模块方法我也写了一点，但由于失败了且没有完成，于是我放在本文的最后，提供一个思路上的借鉴。如果有大佬解决了预览的问题也可以展示以下方法。
:::
...
这里需要构建 3 个仓库，分别储存博客源码、博客文章和合并后的文件。我这边建立了 `Fuwari-Code`、`Fuwari-Posts` 和 `Fuwari-Deploy`，后文分别用这三个仓库代表博客源码仓库、博客文章仓库和合并后的仓库。
...
# 创建 GitHub App 并安装到你的私有仓库

## 创建 GitHub App

打开GitHub-点击 `头像` - `Settings` - `Developer settings`，进入开发者页面。

![](./assets/将Fuwari的代码仓库与文章仓库分离/file-20260116160439810.png)

点击 `GitHub Apps` - `New GitHub App`

![](./assets/将Fuwari的代码仓库与文章仓库分离/image.png)
...
## 安装 GitHub App 到你的私有仓库

回到 GitHub App 详情页 - `Install App` - 找到你的账号，点击 `Install`
...
进入 `Fuwari-Deploy` 仓库 - `Settings` - `Secrets and variables` - `Actions`，添加下面3个 `Secrets`：
...
:::note
这里的私有仓库是指除了 `Fuwari-Deploy` 外的私有仓库。例如我这里除了 `Fuwari-Deploy` 只有 `Fuwari-Posts` 是私有的。
:::

## 创建个人访问令牌

回到 `Developer settings` 页面，选择 `Personal access tokens` - `Tokens (classic)`，点击右上角 `Generate new token` - `Generate new token (classic)`
...
进入 `Fuwari-Deploy` 仓库 - `Settings` - `Secrets and variables` - `Actions`，添加下面这个 `Secrets`：

- `GH_PAT`：粘贴刚刚得到的个人访问令牌字符串
